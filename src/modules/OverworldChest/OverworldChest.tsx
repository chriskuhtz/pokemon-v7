...
const OverworldChestContent = ({
  ...
  const putItemInStorage = (item: ItemType, all?: boolean) => {
    const amount = all ? saveFile.bag[item] : 1;
    patchSaveFileReducer({
      bag: joinInventories(saveFile.bag, { [item]: amount }, true),
    });
    setChestState(joinInventories(chestState, { [item]: amount }));
  };

  const putItemInBag = (item: ItemType, all?: boolean) => {
    const amount = all ? chestState[item] : 1;
    patchSaveFileReducer({
      bag: joinInventories(saveFile.bag, { [item]: amount }),
    });
    setChestState(joinInventories(chestState, { [item]: amount }, true));
  };

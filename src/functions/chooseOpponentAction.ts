...
export const chooseOpponentAction = ({
  ...
  if (
    weatherMove &&
    weather !== weatherMoves[weatherMove.data.name] &&
    !random
  ) {
    return {
      userId: controlled.id,
  ...
  if (
    terrainMove &&
    terrain !== terrainMoves[terrainMove.data.name] &&
    !random
  ) {
    return {
      userId: controlled.id,
  ...
  const healMove = moves.find(
    (m) =>
      m.data.meta.category.name === "heal" && m.data.target.name === "user",
  );

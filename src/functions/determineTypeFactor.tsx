...
export const determineTypeFactor = (
  ...
  if (
    soundBasedMoves.includes(attack.data.name) &&
    attacker.ability === "liquid-voice"
  ) {
    attackType = "water";
  }
  if (attack.data.name === "hidden-power") {
    attackType = getHiddenPowerType(attacker.intrinsicValues);
  }
  if (attack.data.name === "weather-ball") {
    if (weather === "rain") {
      attackType = "water";
    }
    if (weather === "sun") {
  ...
  if (attack.data.name === "terrain-pulse") {
    if (terrain === "electric") {
      attackType = "electric";
    }
    if (terrain === "grassy") {
  ...
  if (attack.data.name === "revelation-dance") {
  ...
  if (
    ...
    ballAndBombMoves.includes(attack.data.name)
  ) {
    if (addMessage) {
      addMessage({
  ...
  if (
    ...
    powderMoves.includes(attack.data.name)
  ) {
    if (addMessage) {
      addMessage({
  ...
  if (attack.data.name === "freeze-dry" && targetTypes.includes("water")) {
    res *= 2;
  }

...
export const KumaQuestsRecord: Record<KumaQuestName, Quest> = {
  ...
  "grow a herb": {
    ...
    researchPoints: 20,
    conditionFunction: (s: SaveFile) => !!s.mileStones.hasGrownAHerb,
    kind: "BULLETIN",
    requiredUpgrade: "berry_farm",
  },
  "grow a apricorn": {
  ...
  "catch a pokemon in an apricorn ball": {
    ...
    conditionFunction: (s) =>
      s.pokemon.some(
        (p) =>
          p.ball !== "poke-ball" &&
          Object.values(apricornTable).includes(p.ball),
      ),
  },
  "catch a pokemon in every type of apricorn ball": {
    category: "EXPLORATION",
    ...
    conditionFunction: (s: SaveFile) =>
      [
        ...
        s.pokedex[o.name].caughtOnRoutes.some(
          (route) =>
            route === "caveW1" || route === "caveW1F1" || route === "caveW1F2",
        ),
      ),
  },
  ...
  "collect 16 badges": {
    ...
    kind: "BULLETIN",
    researchPoints: 200,
    conditionFunction: (s) => s.badges.length >= 16,
    rewardItems: {},

import { makeOverworldItem } from "../../../functions/makeOverworldItem";
import { OverworldMap } from "../../../interfaces/OverworldMap";

export const victoryRoadExitOccupants: OverworldMap["occupants"] = [
  //PORTALS
  {
    id: "victory_road_to_exit",
    type: "ON_STEP_PORTAL",
    x: 10,
    y: 28,
    portal: {
      mapId: "victoryRoad",
      x: 13,
      y: 3,
      orientation: "DOWN",
      forwardFoot: "CENTER1",
    },
    conditionFunction: () => true,
  },
  {
    id: "victory_road_to_pokemonLeague",
    type: "ON_STEP_PORTAL",
    x: 10,
    y: 2,
    portal: {
      mapId: "pokemonLeague",
      x: 5,
      y: 55,
      orientation: "UP",
      forwardFoot: "CENTER1",
    },
    conditionFunction: () => true,
  },
  //OBSTACLES
  //STATUES LEFT SIDE
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 21,
    orientation: "RIGHT",
    dexId: 44,
    dialogue: ["A statue of a Gloom."],
    conditionFunction: (s) => true,
    id: "victory-road_gloom_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 19,
    orientation: "RIGHT",
    dexId: 24,
    dialogue: ["A statue of an Arbok."],
    conditionFunction: (s) => true,
    id: "victory-road_arbok_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 17,
    orientation: "RIGHT",
    dexId: 952,
    dialogue: ["A statue of a Scovillain."],
    conditionFunction: (s) => true,
    id: "victory-road_scovillain_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 15,
    orientation: "RIGHT",
    dexId: 121,
    dialogue: ["A statue of a Starmie."],
    conditionFunction: (s) => true,
    id: "victory-road_starmie_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 13,
    orientation: "RIGHT",
    dexId: 26,
    dialogue: ["A statue of a Raichu."],
    conditionFunction: (s) => true,
    id: "victory-road_raichu_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 11,
    orientation: "RIGHT",
    dexId: 282,
    dialogue: ["A statue of a gardevoir."],
    conditionFunction: (s) => true,
    id: "victory-road_gardevoir_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 9,
    orientation: "RIGHT",
    dexId: 9,
    dialogue: ["A statue of a Blastoise."],
    conditionFunction: (s) => true,
    id: "victory-road_blastoise_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 8,
    y: 7,
    orientation: "RIGHT",
    dexId: 149,
    dialogue: ["A statue of a Dragonite."],
    conditionFunction: (s) => true,
    id: "victory-road_dragonite_statue",
    activeCondition: (s) => true,
  },
  //STATUES RIGHT SIDE
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 21,
    orientation: "LEFT",
    dexId: 164,
    dialogue: ["A statue of a Noctowl."],
    conditionFunction: (s) => true,
    id: "victory-road_noctowl_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 19,
    orientation: "LEFT",
    dexId: 212,
    dialogue: ["A statue of a Scizor."],
    conditionFunction: (s) => true,
    id: "victory-road_scizor_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 17,
    orientation: "LEFT",
    dexId: 38,
    dialogue: ["A statue of a Ninetales."],
    conditionFunction: (s) => true,
    id: "victory-road_ninetales_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 15,
    orientation: "LEFT",
    dexId: 94,
    dialogue: ["A statue of a Gengar."],
    conditionFunction: (s) => true,
    id: "victory-road_gengar_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 13,
    orientation: "LEFT",
    dexId: 68,
    dialogue: ["A statue of a Machamp."],
    conditionFunction: (s) => true,
    id: "victory-road_machamp_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 11,
    orientation: "LEFT",
    dexId: 208,
    dialogue: ["A statue of a Steelix."],
    conditionFunction: (s) => true,
    id: "victory-road_steelix_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 9,
    orientation: "LEFT",
    dexId: 713,
    dialogue: ["A statue of an Avalugg."],
    conditionFunction: (s) => true,
    id: "victory-road_avalugg_statue",
    activeCondition: (s) => true,
  },
  {
    type: "POKEMONSTATUE",
    x: 12,
    y: 7,
    orientation: "LEFT",
    dexId: 621,
    dialogue: ["A statue of a Druddigon."],
    conditionFunction: (s) => true,
    id: "victory-road_druddigon_statue",
    activeCondition: (s) => true,
  },
  //OVERWORLD_MONS
  //TRAINERS
  //ITEMS
  makeOverworldItem({
    mapId: "victoryRoad",
    item: "league-ticket",
    x: 6,
    y: 5,
    amount: 1,
  }),
];
